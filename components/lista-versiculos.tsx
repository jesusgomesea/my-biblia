        ...
        return (
          ...
          >
            ...
            <div className="relative flex shrink-0 self-start gap-0.5">
              ...
              <Link
                href={`/imagem/${livro}/${capitulo}/${versiculo.number}/${versiculo.number}?t=${traducao}`}
                aria-label={`Gerar imagem de ${nomeDoLivro} ${capitulo}:${versiculo.number}`}
                title="Gerar imagem"
                className="rounded p-1.5 font-sans text-muted opacity-60 transition-opacity hover:text-accent hover:opacity-100 focus:opacity-100 focus:outline-none focus-visible:ring-1 focus-visible:ring-accent sm:opacity-0 sm:group-hover:opacity-60"
              >
                <IconeImagem />
              </Link>
              {podeCompartilhar && (
